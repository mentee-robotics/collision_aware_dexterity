...
class FrankaCubeStack(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.cfg["env"]["numObservations"] = 15 + self.z_size #12  n_obs - 3 + 1
    ...
    def create_depth_sensors(self):
        ...
        local_transform_rgb = gymapi.Transform()
        local_transform_rgb.p =  gymapi.Vec3(-0.2, 0.25, -0.1) #gymapi.Vec3(0.0, 0.25, -0.1)   #gymapi.Vec3(0.05, 0, 0.3)    # (-0.026, 0, -0,027)  (0.1, 0, 0.15)
        local_transform_rgb.r = gymapi.Quat.from_axis_angle(gymapi.Vec3(0, -1, 0), np.radians(30)) #60  # gymapi.Quat.from_axis_angle(gymapi.Vec3(0, 1, 0), np.radians(70))  # 45.0))
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        #
        #target_asset = self.gym.create_box(self.sim, *[0.01, 0.01, table_stand_height], table_opts)
        ...
        for i in range(NUM_OBJECTS):
            ...
            #self.cube_heights.append(0.03)
            cubeA_assets.append(cubeA_asset)

            self.object_heights.append(float(line[2]))
            #self.object_heights.append(0.03)

        # Create environments
        for i in range(self.num_envs):
            rand_color = torch.rand(size=(3,))

            cubeA_color = gymapi.Vec3(0.6, 0.1, 0.0) #gymapi.Vec3(rand_color[0], rand_color[1], rand_color[2]) #gymapi.Vec3(0.6, 0.1, 0.0)
    ...
    def _update_states(self):
        self.states.update({
            # Franka
            "q": self._q[:, :],
            "noised_q": self._q[:, :] + 0.02*torch.randn_like(self._q[:, :]), #+ self._noise_for_q,
            ...
            "mean_torque": self.first_step_torques, # self.torques,
            ...
            "in_gripper_pos": (0.3*self._gripper_mover_state[:, :3] + 0.7*self._eef_state[:, :3]),
    ...
    def compute_observations(self):
        ...
        obs = ["q", "gripper_pos_normalized", "gripper_quat", "gripper_torque"] 
        ...
        if self.use_pointcloud or self.use_depth_sensors:
            ...
            if len(first_step_ids) > 0:
                if self.use_pointcloud:
                    ...
                    self.depth_images = torch.cat((rgb_images, rgb_images), dim=1).view(-1, 2 * 64 * 64)
    ...
    def _reset_init_cube_state(self, cube, env_ids, check_valid=True):
        ...
        if check_valid:
            ...
            # sampled_cube_state[:, 0] = centered_cube_xy_state[0] + 0.2
    ...
    def pre_physics_step(self, actions):
        ...
        self.actions = prev_actions + (1/200)* actions * 7.5
        #self.actions = prev_actions + 0.7*self.actions_vel + 0.1 * actions 
        self.actions = self.actions.clip(self.franka_dof_lower_limits, self.franka_dof_upper_limits)

        #self.actions_vel = self.actions - prev_actions
        # print('vel: ', self.actions_vel[-1])
        # print('acc: ', actions[-1])
...
def compute_franka_reward(
    ...
    test_mode = states["cubeB_size"].shape[0] < 64
    ...
    not_laying_x = torch.sum(not_laying_x_ * torch.tensor([0., 0., 1.], device=result.device), dim=-1).abs()
    not_laying_y = torch.sum(not_laying_y_ * torch.tensor([0., 0., 1.], device=result.device), dim=-1).abs()
    ...
    contact_reward = positive_torque * gripper_stuck * close_dist_flag
    ...
    part1 = 1*dist_reward #+ 10*(d < 0.04) #+3 * is_cube_in_frame * dist_reward *(d < 0.05)#dist_reward
    part2  = 20 * cubeA_lifted #80*close_dist_flag*((cubeA_height - cubeA_size) > 0.1)   #1*cubeA_lifted #200*close_dist_flag*(cubeA_height - cubeA_size) #10*cubeA_lifted #*(cubeA_height<0.3)  # close_dist_flag*60. * (cubeA_height)*(cubeA_height<0.3)
    part3 = (50+400.*cubeA_height)*cubeA_lifted #*(cubeA_height>0.2) #0.05*close_dist_flag * states['gripper_torque'].squeeze(-1) #
    part4 = (2*torch.exp(-not_laying_x) + 2*torch.exp(-not_laying_y))*(cubeA_height>=0.1)
    ...
    part7 = 1*contact_reward #-0.4 * distance_of_cube_from_init
    part8 = 1.5*torch.exp(-3*from_side) + 2.5*torch.exp(-3*from_side_b)  #*(d >= 0.04)  # *(cubeA_height>=0.06)
    #print(part1, part2, part3)
    #rewards = torch.sum(torch.cat((part1, part2, part3), -1),-1) # , part2, part3, part4, part5
    rewards = sum([part1, part2]) #part1, part2, part3, part5, 
