...
class VectorizedReplayBuffer:
    def __init__(self, obs_shape, action_shape, capacity, device):
        ...
        self.depth_images = torch.empty((capacity, 32), dtype=torch.float32, device = self.device)
        self.obstacles = torch.empty((capacity, 32), dtype=torch.float32, device = self.device)
    ...
    def add(self, obs, depth_images, obstacles,  action, reward, next_obs, done):
        ...
        if remaining_capacity < num_observations:
            ...
            self.depth_images[0:overflow] = depth_images[-overflow:]
            self.obstacles[0:overflow] = obstacles[-overflow:]
            self.full = True
        ...
        self.depth_images[self.idx: self.idx + remaining_capacity] = depth_images[:remaining_capacity]
        self.obstacles[self.idx: self.idx + remaining_capacity] = obstacles[:remaining_capacity]
    ...
    def sample(self, batch_size):
        ...
        depth_images = self.depth_images[idxs]
        obstacles = self.obstacles[idxs]

        return obses, obstacles, actions, rewards, next_obses, dones
...
class ExperienceBuffer:
    ...
    def _init_from_env_info(self, env_info):
        ...
        self.tensor_dict['obses'] = torch.zeros((obs_base_shape[0], obs_base_shape[1], env_info['observation_space'].shape[0] - 32), device = self.device)
        ...
        self.tensor_dict['depth_images'] = torch.zeros((self.tensor_dict['dones'].shape[0], self.tensor_dict['dones'].shape[1], 2 * 64 * 64), device = self.tensor_dict['dones'].device) #32 is the size of image after resize
        self.tensor_dict['obstacles'] = torch.zeros((self.tensor_dict['dones'].shape[0], self.tensor_dict['dones'].shape[1], 4, 6), device = self.tensor_dict['dones'].device) #32 is the size of image after resize
        
        #self.tensor_dict['mask'] = torch.zeros((self.tensor_dict['dones'].shape[0], self.tensor_dict['dones'].shape[1], 64 * 64), device = self.tensor_dict['dones'].device)
        self.tensor_dict['_cubeA_state'] = torch.zeros((self.tensor_dict['dones'].shape[0], self.tensor_dict['dones'].shape[1], 7), device = self.tensor_dict['dones'].device) 
