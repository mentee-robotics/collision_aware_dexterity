...
class BasePlayer(object):
    ...
    def __init__(self, params):
        ...
        self.depth_images = torch.zeros((8, 2*64 *64), device = self.device) #make it self.num_envs for the first dim
        self.obstacles = torch.zeros((8, 4,6), device = self.device)
        ...
        for _ in range(n_games):
            ...
            for n in range(self.max_steps):
                if has_masks:
                    ...
                    action = self.get_action(obses, self.depth_images, self.obstacles, is_deterministic)

                obses, r, done, info = self.env_step(self.env, action)
                self.depth_images = info['depth_images']
