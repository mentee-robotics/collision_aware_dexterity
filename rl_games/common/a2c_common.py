...
def log_franka_cube_stack_script(experiment_name):
    script_name = '/home/guy/VSprojects/learnable_encoding/isaacgymenvs/tasks/franka_cube_stack.py'
    with open(script_name,'r') as f:
        script_content = f.read()

    with open(experiment_name+'/franka_cube_stack_log.py','w') as f:
...
class A2CBase(BaseAlgorithm):
    ...
    def __init__(self, base_name, params):
        ...
        self.obs = None
        self.depth_images = torch.zeros((self.num_actors, 2 * 64 * 64), device = self.ppo_device) #make it self.num_envs for the first dim
        self.obstacles= torch.zeros((self.num_actors, 4, 6), device = self.ppo_device) #make it self.num_envs for the first dim
        #self.rgb_images = torch.zeros((self.num_actors,3, 64, 64), device = self.ppo_device) #make it self.num_envs for the first dim
        self._cubeA_state =  torch.zeros((self.num_actors, 7), device = self.ppo_device)
        ...
        log_franka_cube_stack_script(self.experiment_dir)
    ...
    def write_stats(self, total_time, epoch_num, step_time, play_time, update_time, a_losses, c_losses, cubeA_state_losses, entropies, kls, last_lr, lr_mul, frame, scaled_time, scaled_play_time, curr_frames):
        ...
        self.writer.add_scalar('losses/cubeA_state_loss', torch_ext.mean_list(cubeA_state_losses).item(), frame)
    ...
    def get_action_values(self, obs, depth_images, obstacles):
    ...
    def get_values(self, obs, depth_images, obstacles):
        with torch.no_grad():
            if self.has_central_value:
                states = obs['states']
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            if self.use_action_masks:
                ...
                res_dict = self.get_action_values(self.obs, self.depth_images, self.obstacles)
            if torch.all(self.obs['obs'] == 0):
                #self.obs['obs'] = self.obs['obs'][:, :12]
                self.obs['obs'] = self.obs['obs'][:, :18]
            ...
            self.depth_images = infos['depth_images']
            self.obstacles = infos['obstacles']
        ...
        last_values = self.get_values(self.obs, self.depth_images, self.obstacles)

        fdones = self.dones.float()
        depth_images = self.experience_buffer.tensor_dict['depth_images']
        obstacles = self.experience_buffer.tensor_dict['obstacles']
        _cubeA_state = self.experience_buffer.tensor_dict['_cubeA_state']        
        ...
        batch_dict['depth_images'] = swap_and_flatten01(depth_images)
        batch_dict['_cubeA_state'] = swap_and_flatten01(_cubeA_state)
        batch_dict['obstacles'] = obstacles
    ...
    def play_steps_rnn(self):
        ...
        for n in range(self.horizon_length):
            ...
            if torch.all(self.obs['obs'] == 0):
                self.obs['obs'] = self.obs['obs'][:, :14]
                
            self.experience_buffer.update_data('obses', n, self.obs['obs'])

            if len(self.depth_images.shape) == 3:
                self.depth_images = self.depth_images.flatten(1, 2)
            self.experience_buffer.update_data('depth_images', n, self.depth_images)
            self.experience_buffer.update_data('obstacles', n, self.obstacles)
            self.experience_buffer.update_data('_cubeA_state', n, self._cubeA_state)
            ...
            self.depth_images = infos['depth_images']
            self.obstacles = infos['obstacles']
            self._cubeA_state = infos['_cubeA_state']
            step_time_end = time.time()
        ...
        batch_dict['depth_images'] = swap_and_flatten01(depth_images)
        batch_dict['_cubeA_state'] = swap_and_flatten01(_cubeA_state)
        batch_dict['obstacles'] = swap_and_flatten01(obstacles)
        batch_dict['played_frames'] = self.batch_size
...
class DiscreteA2CBase(A2CBase):
    ...
    def init_tensors(self):
        ...
        self.tensor_list = self.update_list + ['obses', 'states', 'dones'] #, 'depth_images'
...
class ContinuousA2CBase(A2CBase):
    ...
    def train_epoch(self):
        ...
        for mini_ep in range(0, self.mini_epochs_num):
            ep_kls = []
            for i in range(len(self.dataset)):
                a_loss, c_loss, entropy, kl, last_lr, lr_mul, cmu, csigma, b_loss, cubeA_state_loss = self.train_actor_critic(self.dataset[i])
                a_losses.append(a_loss)
                c_losses.append(c_loss)
                ep_kls.append(kl)
                entropies.append(entropy)
                cubeA_state_losses.append(cubeA_state_loss)
        ...
        return batch_dict['step_time'], play_time, update_time, total_time, a_losses, c_losses, b_losses, cubeA_state_losses, entropies, kls, last_lr, lr_mul
    ...
    def prepare_dataset(self, batch_dict):
        ...
        rnn_states = batch_dict.get('rnn_states', None)
        rnn_masks = batch_dict.get('rnn_masks', None)
        depth_images = batch_dict['depth_images']
        obstacles = batch_dict['obstacles']
        _cubeA_state = batch_dict['_cubeA_state']
        ...
        dataset_dict['depth_images'] = depth_images
        dataset_dict['obstacles'] = obstacles
        dataset_dict['_cubeA_state'] = _cubeA_state
    ...
    def train(self):
        ...
        while True:
            epoch_num = self.update_epoch()
            step_time, play_time, update_time, sum_time, a_losses, c_losses, b_losses, cubeA_state_losses, entropies, kls, last_lr, lr_mul = self.train_epoch()
            total_time += sum_time
            frame = self.frame // self.num_agents
            ...
            if self.rank == 0:
                ...
                self.write_stats(total_time, epoch_num, step_time, play_time, update_time,
                                a_losses, c_losses, cubeA_state_losses, entropies, kls, last_lr, lr_mul, frame,
                                scaled_time, scaled_play_time, curr_frames)
