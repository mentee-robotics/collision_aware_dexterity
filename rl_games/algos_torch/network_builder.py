...
class A2CBuilder(NetworkBuilder):
    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            for m in self.modules():         
                ...
                if isinstance(m, nn.Linear):
                    ...
                    if getattr(m, "bias", None) is not None:
                        torch.nn.init.zeros_(m.bias)    
            #self.auto_encoder = AutoencoderDepth(32, img_size=64, predict_cubeA_state=True, use_q_for_cubeA_prediction=True) #
            self.approx_repset = ApproxRepSet(n_hidden_sets=10, n_elements=20, d=6, n_classes=32, device=None) # n_hidden_sets and n_elements are hyperparameters of repset, d is size of the input vector (in our case 3 poses and 3 dims of the obstacles, n_classes is the output hidden size)
        ...
        def forward(self, obs_dict):
            ...
            #with torch.no_grad():
            #depth_encoding = self.auto_encoder.encode(obs_dict['depth_images'], q=obs[:,:8])
            obstacles_encoding = self.approx_repset(obs_dict['obstacles'])

            obs =  torch.cat((obs, obstacles_encoding ), dim = -1) #(torch.dropout(obs, 0.5,train=False)
